...
async function relayUDP(udpData, ws, hdr) {
    try {
        const dnsSock = connect({ hostname: DNS_SERVER, port: DNS_PORT });
        let outHdr    = hdr;
...
async function processWsRequest(request, gankOverride) {
    ...
    stream.pipeTo(new WritableStream({
        async write(chunk) {
            if (dnsMode) { await relayUDP(chunk, server, null); return; }
            ...
            if (parsed.isUDP) {
                if (parsed.port === DNS_PORT) dnsMode = true;
            ...
            if (dnsMode) { relayUDP(body, server, ackHdr); return; }
